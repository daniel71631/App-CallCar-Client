...
/**
 * Created by daniel on 2017/7/28.
 */

public class SQLitePhonebook extends SQLiteOpenHelper  {

    private final static String DB_NAME = "db_trycontact.db";
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("create table if not exists tb_trycontact(_id integer primary key autoincrement , username , phonenumber)");
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (newVersion > oldVersion) {
            db.execSQL("drop table if exists tb_trycontact");
            onCreate(db);
        }
